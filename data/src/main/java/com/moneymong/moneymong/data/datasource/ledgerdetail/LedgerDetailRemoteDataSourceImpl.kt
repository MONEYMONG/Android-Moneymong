...
class LedgerDetailRemoteDataSourceImpl
    @Inject
    constructor(
        ...
        override suspend fun postLedgerReceiptTransaction(
            detailId: Int,
            body: LedgerReceiptRequest,
        ): Result<Unit> = ledgerDetailApi.postLedgerReceiptTransaction(detailId = detailId, body = body)

        override suspend fun postLedgerDocumentTransaction(
            detailId: Int,
            body: LedgerDocumentRequest,
        ): Result<Unit> = ledgerDetailApi.postLedgerDocumentTransaction(detailId = detailId, body = body)
        ...
        override suspend fun deleteLedgerReceiptTransaction(
            detailId: Int,
            receiptId: Int,
        ): Result<Unit> = ledgerDetailApi.deleteLedgerReceiptTransaction(detailId = detailId, receiptId = receiptId)

        override suspend fun deleteLedgerDocumentTransaction(
            detailId: Int,
            documentId: Int,
        ): Result<Unit> = ledgerDetailApi.deleteLedgerDocumentTransaction(detailId = detailId, documentId = documentId)
